...
$pidPath = Join-Path $PSScriptRoot 'niulai-talk-overlay.pid'
$commandPath = Join-Path $PSScriptRoot 'niulai-talk-command.json'

if (-not (Test-Path -LiteralPath $pidPath)) {
    throw '台词旁车尚未启动。先运行 Start-Niulai.ps1。'
}
...
if ($null -eq $process -or $process.CommandLine -notlike '*NiulaiOverlay.ps1*') {
    throw '台词旁车未在运行。先运行 Start-Niulai.ps1。'
}

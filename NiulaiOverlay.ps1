...
if ([Threading.Thread]::CurrentThread.ApartmentState -ne [Threading.ApartmentState]::STA) {
    throw 'NiulaiOverlay 必须通过 PowerShell -STA 启动。请运行 Start-Niulai.ps1。'
}
...
if (-not ('NiulaiTalk.NativeWindow' -as [type])) {
    Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;
using System.Text;

namespace NiulaiTalk {
    public static class NativeWindow {
        ...
        private static uint GetSafeWindowDpi(IntPtr window) {
            if (window == IntPtr.Zero) return 96;
            try {
                uint dpi = GetDpiForWindow(window);
                return dpi >= 72 && dpi <= 768 ? dpi : 96;
            } catch {
                return 96;
            }
        }

        public static bool TryGetMonitorMetrics(IntPtr petWindow, RECT target, out RECT work, out uint dpi) {
            work = new RECT();
            dpi = GetSafeWindowDpi(petWindow);
        ...
        public static bool PositionBubbleNearPet(IntPtr bubble, IntPtr petWindow, RECT target) {
            ...
            uint dpi = GetSafeWindowDpi(petWindow);
            int gap = Math.Max(4, (int)Math.Round(6 * dpi / 96.0));
            int margin = Math.Max(8, (int)Math.Round(10 * dpi / 96.0));
...
$dialoguePath = Join-Path $packageRoot 'niulai-talk-dialogue.json'
$statePath = Join-Path $packageRoot 'niulai-talk-overlay-state.json'
$layoutStatePath = Join-Path $packageRoot 'niulai-talk-layout-state.json'
$lastMessagePath = Join-Path $packageRoot 'niulai-talk-last-message.json'
$commandPath = Join-Path $packageRoot 'niulai-talk-command.json'
$pointerEventPath = Join-Path $packageRoot 'niulai-talk-pointer-events.json'
...
$window.Title = 'Niulai Speech Overlay'
...
$window.MinWidth = 252
$window.MinHeight = 74
$window.MaxHeight = 190
$window.SizeToContent = [System.Windows.SizeToContent]::Height
$window.Opacity = 0
$window.WindowStartupLocation = [System.Windows.WindowStartupLocation]::Manual

$cardBackground = ([System.Windows.Media.BrushConverter]::new().ConvertFromString('#FFFFF7E6'))
$cardBorderBrush = ([System.Windows.Media.BrushConverter]::new().ConvertFromString('#FFD9A44A'))
$amberBrush = ([System.Windows.Media.BrushConverter]::new().ConvertFromString('#FFE8A72F'))
$deepBrownBrush = ([System.Windows.Media.BrushConverter]::new().ConvertFromString('#FF4A321B'))
$mutedBrownBrush = ([System.Windows.Media.BrushConverter]::new().ConvertFromString('#FF9A671F'))

$cardBorder = New-Object System.Windows.Controls.Border
$cardBorder.CornerRadius = New-Object System.Windows.CornerRadius(15)
$cardBorder.BorderThickness = New-Object System.Windows.Thickness(1.25)
$cardBorder.Padding = New-Object System.Windows.Thickness(13, 11, 14, 11)
$cardBorder.Margin = New-Object System.Windows.Thickness(4)
...
$shadow.Opacity = 0.22
$shadow.Color = ([System.Windows.Media.ColorConverter]::ConvertFromString('#FF4A321B'))
$cardBorder.Effect = $shadow

$panel = New-Object System.Windows.Controls.Grid
$accentColumn = New-Object System.Windows.Controls.ColumnDefinition
$accentColumn.Width = New-Object System.Windows.GridLength(5)
$badgeColumn = New-Object System.Windows.Controls.ColumnDefinition
$badgeColumn.Width = [System.Windows.GridLength]::Auto
...
$panel.ColumnDefinitions.Add($accentColumn)
$panel.ColumnDefinitions.Add($badgeColumn)
$panel.ColumnDefinitions.Add($copyColumn)

$accentBar = New-Object System.Windows.Controls.Border
$accentBar.Width = 5
$accentBar.CornerRadius = New-Object System.Windows.CornerRadius(3)
$accentBar.Background = $amberBrush
$accentBar.Margin = New-Object System.Windows.Thickness(0, 1, 10, 1)
[System.Windows.Controls.Grid]::SetColumn($accentBar, 0)

$badgeBorder = New-Object System.Windows.Controls.Border
$badgeBorder.Width = 38
$badgeBorder.Height = 38
$badgeBorder.CornerRadius = New-Object System.Windows.CornerRadius(19)
$badgeBorder.Background = $amberBrush
$badgeBorder.BorderBrush = $deepBrownBrush
$badgeBorder.BorderThickness = New-Object System.Windows.Thickness(1)
$badgeBorder.Margin = New-Object System.Windows.Thickness(0, 1, 11, 0)
...
$badgeText.Text = '牛'
$badgeText.FontFamily = New-Object System.Windows.Media.FontFamily('Microsoft YaHei UI')
$badgeText.FontSize = 18
$badgeText.FontWeight = [System.Windows.FontWeights]::Bold
$badgeText.Foreground = $deepBrownBrush
...
[System.Windows.Controls.Grid]::SetColumn($copyPanel, 2)

$titleText = New-Object System.Windows.Controls.TextBlock
$titleText.Text = '牛来签 · 片场便笺'
$titleText.FontFamily = New-Object System.Windows.Media.FontFamily('Microsoft YaHei UI')
$titleText.FontSize = 10.8
$titleText.FontWeight = [System.Windows.FontWeights]::Bold
$titleText.Foreground = $mutedBrownBrush
$titleText.Margin = New-Object System.Windows.Thickness(0, 0, 0, 3)
...
$messageText.FontSize = 14.2
$messageText.FontWeight = [System.Windows.FontWeights]::SemiBold
$messageText.Foreground = $deepBrownBrush
$messageText.TextWrapping = [System.Windows.TextWrapping]::Wrap
$messageText.TextTrimming = [System.Windows.TextTrimming]::CharacterEllipsis
$messageText.MaxWidth = 245
$messageText.VerticalAlignment = [System.Windows.VerticalAlignment]::Center

$copyPanel.Children.Add($titleText) | Out-Null
$copyPanel.Children.Add($messageText) | Out-Null
$panel.Children.Add($accentBar) | Out-Null
$panel.Children.Add($badgeBorder) | Out-Null
$panel.Children.Add($copyPanel) | Out-Null
$cardBorder.Child = $panel
$window.Content = $cardBorder
$script:bubbleHandle = [IntPtr]::Zero
$script:bubbleSource = $null
$script:pointerMessageHook = $null
$script:lastLayoutKey = ''
$script:bubbleLayoutState = $null
...
$categoryLabels = @{
    ...
    recovery = '牛来签 · 稳住再来'
    humor = '牛来签 · 牛言牛语'
    interaction = '牛来签 · 你点我答'
    generated = '牛来签 · 随想'
    manual = '牛来签 · 便笺'
}

function Get-AdaptiveBubbleMetrics {
    param(
        [Parameter(Mandatory = $true)][double]$WorkWidthDip,
        [Parameter(Mandatory = $true)][double]$WorkHeightDip
    )

    $safeWidth = [Math]::Max(640, $WorkWidthDip)
    $safeHeight = [Math]::Max(480, $WorkHeightDip)
    $compact = $safeWidth -lt 1200 -or $safeHeight -lt 760
    $widthRatio = if ($safeWidth -lt 1150) { 0.27 } else { 0.23 }
    $width = [Math]::Round([Math]::Max(252, [Math]::Min(368, $safeWidth * $widthRatio)), 1)
    ...
    return [pscustomobject]@{
        Mode = $mode
        Width = $width
        MaxHeight = [Math]::Round([Math]::Max(112, [Math]::Min(190, $safeHeight * 0.27)), 1)
        FontSize = $fontSize
        ...
        PaddingX = $paddingX
        PaddingY = $paddingY
        MessageMaxWidth = [Math]::Max(142, $width - $chromeWidth)
    }
}

function Update-AdaptiveBubbleLayout {
    [double]$workWidthDip = [System.Windows.SystemParameters]::WorkArea.Width
    [double]$workHeightDip = [System.Windows.SystemParameters]::WorkArea.Height
    [uint32]$dpi = 96
    $layoutSource = 'primary-work-area'

    $target = New-Object NiulaiTalk.NativeWindow+RECT
    $petWindow = [IntPtr]::Zero
    if ([NiulaiTalk.NativeWindow]::TryGetCodexPetWindow([ref]$petWindow, [ref]$target)) {
        $work = New-Object NiulaiTalk.NativeWindow+RECT
        if ([NiulaiTalk.NativeWindow]::TryGetMonitorMetrics($petWindow, $target, [ref]$work, [ref]$dpi)) {
            $workWidthDip = ($work.Right - $work.Left) * 96.0 / [Math]::Max(72, [double]$dpi)
            $workHeightDip = ($work.Bottom - $work.Top) * 96.0 / [Math]::Max(72, [double]$dpi)
            $layoutSource = 'pet-monitor'
        }
    }

    $metrics = Get-AdaptiveBubbleMetrics -WorkWidthDip $workWidthDip -WorkHeightDip $workHeightDip
    $layoutKey = '{0}:{1}:{2}:{3}' -f $metrics.Mode, [Math]::Round($workWidthDip), [Math]::Round($workHeightDip), $dpi
    if ($layoutKey -eq $script:lastLayoutKey) {
        return
    }

    $script:lastLayoutKey = $layoutKey
    $window.Width = $metrics.Width
    $window.MaxHeight = $metrics.MaxHeight
    $cardBorder.Padding = New-Object System.Windows.Thickness($metrics.PaddingX, $metrics.PaddingY, ($metrics.PaddingX + 1), $metrics.PaddingY)
    $badgeBorder.Width = $metrics.BadgeSize
    $badgeBorder.Height = $metrics.BadgeSize
    $badgeBorder.CornerRadius = New-Object System.Windows.CornerRadius($metrics.BadgeSize / 2)
    $badgeText.FontSize = $metrics.BadgeFontSize
    $titleText.FontSize = $metrics.TitleFontSize
    $messageText.FontSize = $metrics.FontSize
    $messageText.MaxWidth = $metrics.MessageMaxWidth
    $window.InvalidateMeasure()
    $window.UpdateLayout()

    $script:bubbleLayoutState = [ordered]@{
        mode = $metrics.Mode
        source = $layoutSource
        dpi = [int]$dpi
        workWidthDip = [Math]::Round($workWidthDip, 1)
        workHeightDip = [Math]::Round($workHeightDip, 1)
        bubbleWidthDip = $metrics.Width
        bubbleMaxHeightDip = $metrics.MaxHeight
        fontSizeDip = $metrics.FontSize
        updatedAt = (Get-Date).ToString('o')
    }
    Save-Json -Path $layoutStatePath -Value $script:bubbleLayoutState
}

function Update-BubblePosition {
    Update-AdaptiveBubbleLayout
    ...
    if ([NiulaiTalk.NativeWindow]::TryGetCodexPetWindow([ref]$petWindow, [ref]$target)) {
        if ($script:bubbleHandle -ne [IntPtr]::Zero -and
            [NiulaiTalk.NativeWindow]::PositionBubbleNearPet($script:bubbleHandle, $petWindow, $target)) {
            return
        }
...
function Get-PetHitBounds {
    $target = New-Object NiulaiTalk.NativeWindow+RECT
    if (-not [NiulaiTalk.NativeWindow]::TryGetCodexPetRect([ref]$target)) {
        return $null
    }
    ...
    [void][NiulaiTalk.NativeWindow]::GetPetHorizontalBand(
        $target,
        [ref]$petLeftFraction,
...
function Write-PetInteractionState {
    ...
    [System.IO.File]::WriteAllText(
        (Join-Path $packageRoot 'niulai-talk-interaction-state.json'),
...
function Register-PetClick {
    ...
    if ($null -ne $script:pendingPetClickAt) {
        ...
        if ($elapsed -le $script:doubleClickMilliseconds -and $distance -lt 16) {
            ...
            # The bridge now owns one event per physical click. Keep only a
            # short debounce tail after a completed double-click.
            $script:ignorePetClicksUntil = $At.AddMilliseconds(180)
            $script:lastPetClickAt = $At
            Write-PetInteractionState -Action 'double' -At $At
            [void][NiulaiTalk.NativeWindow]::ActivateCodexMainWindow()
            return
        }
    }
    ...
    # Speak on the first click immediately. The pending timestamp remains only
    # to recognize a possible second click; it no longer delays feedback.
...
function Receive-PetPointerEvents {
    ...
    try {
        ...
        foreach ($event in @($state.events)) {
            ...
            if ($eventKind -eq 'click' -or $eventKind -eq 'drag') {
            ...
            } elseif ($eventKind -eq 'drag') {
                $script:pendingPetClickAt = $null
                $script:lastPetDragAt = $eventAt
                $script:lastPetHoverAt = $eventAt
                Write-PetInteractionState -Action 'drag' -At $eventAt
...
$script:processedPointerEventIds = @()
$script:systemDoubleClickMilliseconds = [Math]::Max(200, [int][NiulaiTalk.NativeWindow]::GetDoubleClickTime())
...
$interactionTimer.Add_Tick({
    $cursor = New-Object NiulaiTalk.NativeWindow+POINT
    if (-not [NiulaiTalk.NativeWindow]::GetCursorPos([ref]$cursor)) {
        return
    }
    ...
    $now = Get-Date
    $leftButtonState = [int][NiulaiTalk.NativeWindow]::GetAsyncKeyState(1)
    ...
    if ($overPet -and -not $leftButtonDown) {
        ...
        if (-not $script:mouseWasDown) {
            $script:foregroundBeforePetClick = [NiulaiTalk.NativeWindow]::GetForegroundWindow()
        }
...
function Show-Message {
    ...
    $titleText.Text = if ($categoryLabels.ContainsKey($messageCategory)) {
        [string]$categoryLabels[$messageCategory]
    } else {
        '牛来签 · 片场便笺'
    }
    $window.InvalidateMeasure()
...
$window.Add_SourceInitialized({
    ...
    [NiulaiTalk.NativeWindow]::MakeClickThrough($script:bubbleHandle)

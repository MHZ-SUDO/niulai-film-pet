...
public static class NiulaiInputNative
{
    ...
    // Treat small touchpad/high-DPI jitter as a click. The PowerShell fallback
    // uses the same 16-pixel boundary, so hook and fallback semantics match.
    private const long BODY_DRAG_THRESHOLD_SQUARED = 256L;
    ...
    private static volatile bool MouseBodyExternalDragRelay;
    ...
    public static bool StartMouseHook()
    {
        lock (MouseHookSync)
        {
            ...
            MouseHookThread.Name = "Niulai mouse capture";
    ...
    public static bool IsMouseHookLeftButtonHeld()
    {
        return MouseLeftButtonHeld;
    }

    public static void SetMouseBodyExternalDragRelay(bool enabled)
    {
        MouseBodyExternalDragRelay = enabled;
    }
...
if ($HookProbe) {
    $started = [NiulaiInputNative]::StartMouseHook()
    Start-Sleep -Milliseconds 100
    $active = [NiulaiInputNative]::IsMouseHookActive()
    $errorCode = [NiulaiInputNative]::GetMouseHookLastError()
    $exception = [NiulaiInputNative]::GetMouseHookException()
    [NiulaiInputNative]::StopMouseHook()
    [pscustomobject]@{
        started = $started
        active = $active
        errorCode = $errorCode
...
function Get-PetAutomationLayout {
    ...
    if ($Overlay -eq [IntPtr]::Zero -or -not [NiulaiInputNative]::IsWindow($Overlay)) {
        return $null
    }
...
function Get-OverlaySnapshot {
    ...
    $rect = New-Object NiulaiInputNative+RECT
    $cursor = New-Object NiulaiInputNative+POINT
    if (-not [NiulaiInputNative]::GetWindowRect($Overlay, [ref]$rect)) {
        return $null
    }

    $hasCursor = [NiulaiInputNative]::GetCursorPos([ref]$cursor)
    $inside = $hasCursor -and
        $cursor.X -ge $rect.Left -and $cursor.X -lt $rect.Right -and
        $cursor.Y -ge $rect.Top -and $cursor.Y -lt $rect.Bottom
    $extendedStyle = [NiulaiInputNative]::GetWindowLongPtr($Overlay, -20).ToInt64()
    $transparent = ($extendedStyle -band 0x20) -ne 0
    [uint32]$ownerProcessId = 0
    [void][NiulaiInputNative]::GetWindowThreadProcessId($Overlay, [ref]$ownerProcessId)
    [double]$petBandLeft = 0.42
    [double]$petBandRight = 0.58
    [void][NiulaiInputNative]::GetPetHorizontalBand(
        $Overlay,
        [ref]$petBandLeft,
        [ref]$petBandRight
    )
...
function Send-OverlayMouseMove {
    ...
    return [NiulaiInputNative]::PostMessage(
        $Snapshot.overlay,
        0x0200,
        [IntPtr]::Zero,
        [NiulaiInputNative]::MakeMouseLParam($localX, $localY)
    )
}
...
function Send-NativePetPointerMessage {
    param(
        $Snapshot,
        [ValidateSet(0x0200, 0x0201, 0x0202)]
        [int]$Message,
        [int]$X,
        [int]$Y
    )
    ...
    $buttonState = if ($Message -eq 0x0202) { [IntPtr]::Zero } else { [IntPtr]::new(1) }
    return [NiulaiInputNative]::PostMessage(
        $Snapshot.overlay,
        [uint32]$Message,
        $buttonState,
...
function Update-PetBodyClickGuard {
    ...
    if ($null -eq $Snapshot -or $Snapshot.overlay -eq [IntPtr]::Zero) {
        [NiulaiInputNative]::ClearPetBodyClickGuard()
        return
    }
    ...
    [NiulaiInputNative]::ConfigurePetBodyClickGuard(
        $Snapshot.overlay,
        $guardLeft,
        $guardTop,
        $guardRight,
...
function Write-PointerEventFile {
    $payload = [ordered]@{
        ...
        hookReleasePairingGraceMilliseconds = $script:hookReleasePairingGraceMilliseconds
        hookReleaseTimeoutCount = $script:hookReleaseTimeoutCount
        dragTransport = $script:dragTransport
        nativeDragRelayStartCount = $script:nativeDragRelayStartCount
        nativeDragRelayFailureCount = $script:nativeDragRelayFailureCount
        nativeBodyInputSuppressionCount = [NiulaiInputNative]::GetBodyInputSuppressionCount()
...
function Notify-SpeechOverlayPointerEvent {
    try {
        $speechWindow = [NiulaiInputNative]::FindWindow(
            [string]$null,
            'Niulai Speech Overlay'
        )
        if ($speechWindow -ne [IntPtr]::Zero) {
            [void][NiulaiInputNative]::PostMessage(
                $speechWindow,
                0x8001,
                [IntPtr]::Zero,
                [IntPtr]::Zero
...
function Publish-PetBodyClick {
    ...
    if ($written) {
        Notify-SpeechOverlayPointerEvent
    }
    ...
    if ($ForegroundBefore -ne [IntPtr]::Zero -and
        [NiulaiInputNative]::GetForegroundWindow() -ne $ForegroundBefore) {
        try {
            [void][NiulaiInputNative]::RestoreForegroundWithoutChangingWindowState($ForegroundBefore)
        }
        catch {
...
function Publish-PetBodyDrag {
    ...
    $event = [ordered]@{
        ...
        deltaX = $deltaX
        deltaY = $deltaY
        distance = [Math]::Round([Math]::Sqrt(($deltaX * $deltaX) + ($deltaY * $deltaY)), 1)
        direction = if ([Math]::Abs($deltaX) -ge [Math]::Abs($deltaY)) {
            if ($deltaX -ge 0) { 'right' } else { 'left' }
        } elseif ($deltaY -ge 0) {
            'down'
        } else {
            'up'
...
function Set-OverlayTransparent {
    ...
    $style = [NiulaiInputNative]::GetWindowLongPtr($Overlay, -20).ToInt64()
    ...
    [void][NiulaiInputNative]::SetWindowLongPtr($Overlay, -20, [IntPtr]::new($desired))
    [void][NiulaiInputNative]::SetWindowPos(
        $Overlay,
        [IntPtr]::new(-1),
        0,
        0,
...
if ($ProbeOnly) {
    $overlay = [NiulaiInputNative]::FindOverlay()
...
$mutex = New-Object Threading.Mutex($false, 'Local\NiulaiFilmPetCodexInputBridge')
$ownsMutex = $false
...
try {
    ...
    $nativeDragRelayActive = $false
    $lastNativeDragRelayAt = [DateTime]::MinValue
    $bodyHookReleaseMissingSince = [DateTime]::MinValue
    ...
    $foregroundBeforeMouseDown = [NiulaiInputNative]::GetForegroundWindow()
    $script:pointerEventPath = Join-Path $PSScriptRoot 'niulai-talk-pointer-events.json'
    ...
    $script:hookReleasePairingGraceMilliseconds = 250
    $script:hookReleaseTimeoutCount = 0
    $script:nativeDragRelayStartCount = 0
    $script:nativeDragRelayFailureCount = 0
    $mouseHookStarted = [NiulaiInputNative]::StartMouseHook()
    $script:dragTransport = if ($mouseHookStarted) { 'native-pointer-relay' } else { 'native-fallback' }
    [NiulaiInputNative]::SetMouseBodyExternalDragRelay($mouseHookStarted)
    $script:pointerCaptureMode = if ($mouseHookStarted) { 'low-level-hook' } else { 'poll-fallback' }
    $script:pointerHookError = [NiulaiInputNative]::GetMouseHookLastError()
    ...
    while ($true) {
        ...
        if (($hookNow - $lastHookHealthAt).TotalSeconds -ge 3) {
            ...
            if (-not [NiulaiInputNative]::IsMouseHookActive()) {
                $mouseHookStarted = [NiulaiInputNative]::StartMouseHook()
                $script:pointerCaptureMode = if ($mouseHookStarted) { 'low-level-hook' } else { 'poll-fallback' }
                $script:dragTransport = if ($mouseHookStarted) { 'native-pointer-relay' } else { 'native-fallback' }
                [NiulaiInputNative]::SetMouseBodyExternalDragRelay($mouseHookStarted)
        ...
        $hookEvents = @()
        if ($mouseHookStarted) {
            $hookEvents = @([NiulaiInputNative]::DrainMouseHookEvents())
        }
        ...
        if ($null -eq $snapshot) {
            ...
            $nativeDragRelayActive = $false
            $lastNativeDragRelayAt = [DateTime]::MinValue
            [NiulaiInputNative]::SetMouseBodyExternalDragRelay($false)
            $bodyHookReleaseMissingSince = [DateTime]::MinValue
            $bodyReleaseCandidateAt = $null
            $script:petAutomationLayout = $null
            $overlay = [IntPtr]::Zero
        ...
        $leftButtonState = [int][NiulaiInputNative]::GetAsyncKeyState(1)
        ...
        if ($mouseHookStarted -and $hookEvents.Count -gt 0) {
            foreach ($hookEvent in $hookEvents) {
                ...
                if ($hookMessage -eq 0x0201 -and $hookBodyGestureOwned) {
                    ...
                    $nativeDragRelayActive = $false
                    $lastNativeDragRelayAt = [DateTime]::MinValue
                    [NiulaiInputNative]::SetMouseBodyExternalDragRelay($true)
                    $script:dragTransport = 'native-pointer-relay'
                    $bodyDownX = $hookX
                    $bodyDownY = $hookY
                    $bodyMaxDistance = 0.0
                    $bodyDragStarted = $false
                    ...
                    if ($bodyForegroundBefore -eq [IntPtr]::Zero) {
                        $bodyForegroundBefore = [NiulaiInputNative]::GetForegroundWindow()
                    }
                    ...
                    $dragWasStarted = $bodyDragStarted
                    $bodyMaxDistance = [Math]::Max($bodyMaxDistance, $hookDistance)
                    $bodyDragStarted = $bodyDragStarted -or $hookBodyDragStarted
                    $dragLastCursorX = $hookX
                    $dragLastCursorY = $hookY
                    $dragLastMotionAt = $hookEventAt
                    if ($bodyDragStarted -and -not $dragWasStarted) {
                        $relaySnapshot = Get-OverlaySnapshot -Overlay $overlay
                        if ($null -eq $relaySnapshot) { $relaySnapshot = $snapshot }
                        $relayDown = Send-NativePetPointerMessage -Snapshot $relaySnapshot -Message 0x0201 -X $bodyDownX -Y $bodyDownY
                        $relayMove = Send-NativePetPointerMessage -Snapshot $relaySnapshot -Message 0x0200 -X $hookX -Y $hookY
                        if ($relayDown -and $relayMove) {
                            $nativeDragRelayActive = $true
                            $lastNativeDragRelayAt = $hookEventAt
                            $script:nativeDragRelayStartCount++
                        } else {
                            $script:nativeDragRelayFailureCount++
                            $script:dragTransport = 'helper-setwindowpos-fallback'
                            [NiulaiInputNative]::SetMouseBodyExternalDragRelay($false)
                        }
                    } elseif ($bodyDragStarted -and $nativeDragRelayActive -and
                        ($hookEventAt - $lastNativeDragRelayAt).TotalMilliseconds -ge 16) {
                        $relaySnapshot = Get-OverlaySnapshot -Overlay $overlay
                        if ($null -eq $relaySnapshot) { $relaySnapshot = $snapshot }
                        if (-not (Send-NativePetPointerMessage -Snapshot $relaySnapshot -Message 0x0200 -X $hookX -Y $hookY)) {
                            $script:nativeDragRelayFailureCount++
                        }
                        $lastNativeDragRelayAt = $hookEventAt
                    }
                    ...
                    $bodyMaxDistance = [Math]::Max($bodyMaxDistance, $hookDistance)
                    $bodyDragStarted = $bodyDragStarted -or $hookBodyDragStarted
                    if ($bodyDragStarted -and $nativeDragRelayActive) {
                        $relaySnapshot = Get-OverlaySnapshot -Overlay $overlay
                        if ($null -eq $relaySnapshot) { $relaySnapshot = $snapshot }
                        $relayMove = Send-NativePetPointerMessage -Snapshot $relaySnapshot -Message 0x0200 -X $hookX -Y $hookY
                        $relayUp = Send-NativePetPointerMessage -Snapshot $relaySnapshot -Message 0x0202 -X $hookX -Y $hookY
                        if (-not ($relayMove -and $relayUp)) {
                            $script:nativeDragRelayFailureCount++
                        }
                    }
                    if (-not $bodyDragStarted -and $hookDistance -lt 16 -and $bodyMaxDistance -lt 16) {
                        ...
                        Publish-PetBodyDrag `
                            ...
                            -X $hookX `
                            -Y $hookY `
                            -NativeRelay $nativeDragRelayActive
                    }
                    $bodyClickArmed = $false
                    $bodyHookReleaseMissingSince = [DateTime]::MinValue
                    $dragCaptureActive = $false
        ...
        if ($mouseHookStarted -and $bodyClickArmed) {
            $hookButtonHeld = [NiulaiInputNative]::IsMouseHookLeftButtonHeld()
            if ($hookButtonHeld) {
                $bodyHookReleaseMissingSince = [DateTime]::MinValue
            } elseif ($bodyHookReleaseMissingSince -eq [DateTime]::MinValue) {
                # The hook enqueues WM_LBUTTONUP before it clears its held flag.
                # A fast click can therefore put down and up in adjacent drain
                # batches. Give that already-queued up record one bounded
                # pairing window before treating it as genuinely lost.
                $bodyHookReleaseMissingSince = [DateTime]::UtcNow
            } elseif (([DateTime]::UtcNow - $bodyHookReleaseMissingSince).TotalMilliseconds -ge
                $script:hookReleasePairingGraceMilliseconds) {
                # Fail closed only when the hook itself says the button is up
                # and no paired release record arrived during the grace window.
                $bodyClickArmed = $false
                $bodyDragStarted = $false
                if ($nativeDragRelayActive) {
                    [void](Send-NativePetPointerMessage -Snapshot $snapshot -Message 0x0202 -X $snapshot.cursorX -Y $snapshot.cursorY)
                    [void][NiulaiInputNative]::PostMessage($snapshot.overlay, 0x001F, [IntPtr]::Zero, [IntPtr]::Zero)
                }
                $nativeDragRelayActive = $false
                $lastNativeDragRelayAt = [DateTime]::MinValue
                [NiulaiInputNative]::SetMouseBodyExternalDragRelay($false)
                ...
                $script:petAutomationLayout = $null
                $lastAutomationLookup = [DateTime]::MinValue
                $script:hookReleaseTimeoutCount++
                [void](Write-PointerEventFile)
            }
        }
        ...
        if (-not $nativeDragRelayActive) {
            # Do not interleave a button-up hover move with the synthetic
            # button-held stream that drives Codex's native drag animation.
        ...
        if ($leftButtonDown -and -not $mouseWasDown -and (Test-PetButtonZone -Snapshot $snapshot)) {
            $renderer = [NiulaiInputNative]::FindRenderer($overlay)
            $hit = [NiulaiInputNative]::WindowFromPoint(
                [NiulaiInputNative+POINT]@{ X = $snapshot.cursorX; Y = $snapshot.cursorY }
            )
            $hitRoot = if ($hit -ne [IntPtr]::Zero) {
                [NiulaiInputNative]::GetAncestor($hit, 2)
            } else {
                [IntPtr]::Zero
            }
        ...
        if (-not $leftButtonDown -and $mouseWasDown -and $syntheticClickPending) {
            ...
            if ($distance -lt 16 -and (Test-PetButtonZone -Snapshot $snapshot)) {
                [void](Set-OverlayTransparent -Overlay $overlay -Enabled $false -ReferenceStyle $forcedOriginalStyle)
                [NiulaiInputNative]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
                Start-Sleep -Milliseconds 60
                [NiulaiInputNative]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
            }
            $syntheticClickPending = $false
        }

        if (-not $leftButtonDown -and -not $mouseWasDown) {
            $idleForeground = [NiulaiInputNative]::GetForegroundWindow()
    ...
    [NiulaiInputNative]::ClearPetBodyClickGuard()
    if ($mouseHookStarted -or [NiulaiInputNative]::IsMouseHookActive()) {
        [NiulaiInputNative]::StopMouseHook()
    }
